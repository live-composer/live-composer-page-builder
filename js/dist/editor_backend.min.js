...
document.addEventListener('moduleEdit', function (customEvent) {
  ...
  window.dslc_module_options_show(currentModuleId); // Cover up other modules with semi transp cover
  ...
  if (pasteModuleProperies) {
    ...
    if (pasteModuleProperies.module_id === currentModuleEl.dataset.module) {
      ...
      if (currentModuleProperties) {
        ...
        for (var _propertyId in currentModuleProperties) {
          // If property is missing in pasted data make it empty.
          if (_propertyId.includes('css_')) {
            if (currentModuleProperties[_propertyId] !== pasteModuleProperies[_propertyId]) {
              currentModuleProperties[_propertyId] = '';
            }
...
var adjustZindex = function adjustZindex() {
  LiveComposer.Builder.PreviewAreaDocument.on({
    mouseenter: function mouseenter(e) {
      // Adjust z-index.
      jQuery('.dslca-modules-section-manage', LiveComposer.Builder.PreviewAreaDocument).css("z-index", "99998"); // Adjust module editing controls for too small elements.

      var moduleEl = e.target.closest("[data-module-id]");
      var elementHeight = moduleEl.offsetHeight;
      var elementWidth = moduleEl.offsetWidth;
...
function dslc_reload_module(moduleID, callback) {
  window.dslc_module_output_reload(moduleID, callback);
}
...
jQuery(document).on('editorFrameLoaded', function () {
  ...
  */
  ...
  function dslc_modules_area_vertical_align_set(areaJQ, newSetting) {
    if (window.dslcDebug) console.log('dslc_modules_area_vertical_align_set'); // Generate new class based on align option.

    var newClass = 'dslc-valign-' + newSetting; // Remove previously set align classes,
    // add new class and set the data-valign attr for storing data.

    areaJQ.removeClass('dslc-valign-top dslc-valign-middle dslc-valign-bottom').addClass(newClass).data('valign', newSetting); // Call other functions
...
var resetResponsiveOptions = function resetResponsiveOptions() {
  ...
  if ('tablet_responsive' === currentTab || 'phone_responsive' === currentTab) {
    ...
    activeSettings.forEach(function (element) {
      var optionId = element.querySelector('.dslca-module-edit-field').dataset.id; // Don't want to reset value of the breakpoint enabler.
...
window.dslc_module_options_font = function () {
  ...
  jQuery(document).off('keyup', '.dslca-module-edit-field-font').on('keyup', '.dslca-module-edit-field-font', onKeyUp); // Keyup ( left arrow, right arrow, else )

  function onKeyUp(e) {
    console.log("1 e.which:");
    console.log(e.which);
    var dslcField,
        dslcOption,
        dslcVal,
        ...
        dslcFont;
    dslcField = jQuery(this);
    dslcOption = dslcField.closest('.dslca-module-edit-option'); // Arrow Up.
  ...
  ;
  jQuery(document).off('keydown', '.dslca-module-edit-field-font').on('keydown', '.dslca-module-edit-field-font', onPushEnter); // Key press ( enter = 13 and tab = 9 )

  function onPushEnter(e) {
    console.log("2 e.which:");
    console.log(e.which);
