...
function dslc_on_tab_add(event) {
  ...
  dslc_tabs_generate_code(dslcTabs); // openModuleForEditing( this[0] );
...
function dslc_on_tab_nav(event) {
  ...
  dslcTabsContent = jQuery('.dslc-tabs-content', dslcTabs);
  dslcTabContent = jQuery('.dslc-tabs-tab-content', dslcTabs);
  dslcTabIndex = jQuery(event.currentTarget).index(); // Tabs nav

  jQuery('.dslc-tabs-nav-hook.dslc-active', dslcTabs).removeClass('dslc-active');
  jQuery(event.currentTarget).addClass('dslc-active'); // Tabs content
...
jQuery(document).on('click', '.dslca-add-new-tab-hook', function (event) {
  return dslc_on_tab_add(event);
});
jQuery(document).on('click', '.dslca-delete-tab-hook', function (event) {
  return dslc_on_tab_delete(event);
});
jQuery(document).on('click', '.dslc-tabs-nav-hook', function (event) {
  return dslc_on_tab_nav(event);
});
      ...
      if (scrollTop < 50 || scrollTop % 10 === 0) {
        if (scrollTop >= 0) {
          headers.each(function (index, el) {
            ...
            var isRelative = scrollTop <= el.exTop; // scroll down
...
jQuery(document).ready(function ($) {
  ...
  dslc_parallax();
  dslc_init_lightbox();
  window.dslc_sticky_row(); // No need to wait for jQuery(window).load.
