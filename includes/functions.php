	...
	foreach($options_arr as $option_arr) {
		...
		if (isset($option_arr['affect_on_change_el']) && isset($option_arr['affect_on_change_rule'])) {
			...
			foreach($affect_els_arr as $affect_el_arr) {
				...
				if (isset($option_arr['section']) && $option_arr['section'] == 'responsive') {
					...
					case __('tablet', 'live-composer-page-builder'):
						if (isset($settings['css_res_t']) && $settings['css_res_t'] == 'enabled') $affect_el.= 'body.dslc-res-tablet #dslc-content #dslc-module-' . $settings['module_instance_id'] . ' ' . $affect_el_arr;
						break;

					case __('phone', 'live-composer-page-builder'):
						if (isset($settings['css_res_p']) && $settings['css_res_p'] == 'enabled') $affect_el.= 'body.dslc-res-phone #dslc-content #dslc-module-' . $settings['module_instance_id'] . ' ' . $affect_el_arr;
						break;
					}
				}
				else {
			...
			if ($option_arr['type'] == 'checkbox' && $option_arr['refresh_on_change'] == false) {
				...
				if (in_array('top', $checkbox_arr)) $checkbox_val.= 'solid ';
				else $checkbox_val.= 'none ';
				if (in_array('right', $checkbox_arr)) $checkbox_val.= 'solid ';
				else $checkbox_val.= 'none ';
				if (in_array('bottom', $checkbox_arr)) $checkbox_val.= 'solid ';
				else $checkbox_val.= 'none ';
				if (in_array('left', $checkbox_arr)) $checkbox_val.= 'solid ';
				else $checkbox_val.= 'none ';
...
function dslc_body_class($classes)
{
	...
	if ($proceed == false) return $classes;

	// If page in LC mode, force the class

	if (isset($_GET['dslc']) && $_GET['dslc'] == 'active') $has_lc_content = true;
...
function dslc_set_defaults($new_defaults, $options)
{
	...
	if (!$new_defaults) return $options;
...
{
	global $dslc_var_modules;
	if (dslc_get_option($module_ID, 'dslc_plugin_options_features') == 'disabled') return false;
	elseif ($check_registered == true && !isset($dslc_var_modules[$module_ID])) return false;
	else return true;
}
