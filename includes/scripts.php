...
final class DSLC_Scripts{
	 ...
	 * @since 1.0
	 */
	public static function dslc_load_scripts() {
	...
	public function init() {
		...
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'dslc_load_scripts' ) );
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'dslc_load_fonts' ) );

		add_action( 'admin_enqueue_scripts', array( __CLASS__, 'dslc_load_editing_scripts' ) );
		add_action( 'admin_enqueue_scripts', array( __CLASS__, 'dslc_load_admin_scripts' ) );

		add_action( 'admin_footer-plugins.php', array( __CLASS__, 'dslc_inline_js_plugin_title' ) );
	}

	/**
	 * Load CSS and JS files in wp-admin page
	 *
	 * @since 1.1
	 */
	public static function dslc_load_editing_scripts() {
		...
		if ( is_user_logged_in() && current_user_can( DS_LIVE_COMPOSER_CAPABILITY ) ) {
			...
			wp_enqueue_script( 'mq-outerhide-js', DS_LIVE_COMPOSER_URL . 'js/libs/mq-outer-hide.js', array( 'jquery' ), DS_LIVE_COMPOSER_VER );
			wp_enqueue_script( 'mq-mutation-observer-js', DS_LIVE_COMPOSER_URL . 'js/libs/mq-mutation-observer.js', array(), DS_LIVE_COMPOSER_VER );
			wp_enqueue_script( 'util-js', DS_LIVE_COMPOSER_URL . 'js/common/util.class.js', array( 'jquery' ), DS_LIVE_COMPOSER_VER );
			wp_enqueue_script( 'base64', DS_LIVE_COMPOSER_URL . 'js/libs/base64.js', array(), DS_LIVE_COMPOSER_VER );
			wp_enqueue_script( 'modernizr', DS_LIVE_COMPOSER_URL . 'js/libs/modernizr-custom.js', array(), DS_LIVE_COMPOSER_VER );
			...
			if ( ! SCRIPT_DEBUG ) {
				...
				wp_enqueue_script( 'dslc-builder-main-js', DS_LIVE_COMPOSER_URL . 'js/builder.main.min.js', array( 'dslc-main-js' ), DS_LIVE_COMPOSER_VER );
			} else {

				wp_enqueue_script( 'dslc-builder-main-js', DS_LIVE_COMPOSER_URL . 'js/builder.main.js', array( 'dslc-main-js' ), DS_LIVE_COMPOSER_VER );
	 ...
	 *
	 * @param string $hook 	 some hook
	 * @since 1.0
	 */
	public static function dslc_load_admin_scripts( $hook ) {
		...
		if ( false !== strpos( $hook, 'dslc_plugin_options' ) || 'dslc_plugin_options' === get_admin_page_parent() || false !== strpos( $hook, 'dslc_getting_started' ) ) {
		...
		// Yoast SEO.
		if ( 'post-editing' === $current_screen && defined( 'WPSEO_VERSION' ) ) {
